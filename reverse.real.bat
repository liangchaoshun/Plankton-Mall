...
REM Automatically obtain the serial number of the currently connected physical device.
REM If there are multiple physical devices, obtain the last one.

set DEVICE=
for /f "tokens=1" %%i in ('adb devices ^| findstr /v "List" ^| findstr /v "emulator" ^| findstr /r "device$"') do (
    set DEVICE=%%i
)
